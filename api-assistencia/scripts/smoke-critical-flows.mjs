...
async function cleanupDatabase() {
  ...
  try {
    ...
    const vendaIds = [created.vendaId, created.vendaCanceladaId].filter(Boolean);
    ...
    if (vendaIds.length) {
      ...
      const [itemResult] = await connection.execute(`DELETE FROM venda_itens WHERE venda_id IN (${placeholders})`, vendaIds);
      cleanup.venda_itens += itemResult.affectedRows;
      const [vendaResult] = await connection.execute(`DELETE FROM vendas WHERE id IN (${placeholders}) AND observacao LIKE ?`, [
        ...vendaIds,
        `%${marker}%`
      ]);
      cleanup.vendas += vendaResult.affectedRows;
    }
    ...
    const checks = {
      ...
      movimentacoes_estoque:
        created.produtoId || created.orcamentoId
          ...
          : 0,
      vendas: created.vendaId
        ? (await queryOne(connection, 'SELECT COUNT(*) count FROM vendas WHERE id = ? AND observacao LIKE ?', [created.vendaId, `%${marker}%`])).count
        : 0,
      venda_itens:
        created.vendaId || created.vendaCanceladaId
          ? (
              await queryOne(connection, 'SELECT COUNT(*) count FROM venda_itens WHERE venda_id IN (?, ?)', [
                created.vendaId || 0,
                created.vendaCanceladaId || 0
              ])
            ).count
        : 0,
      vendas_canceladas: created.vendaCanceladaId
        ? (
            await queryOne(connection, 'SELECT COUNT(*) count FROM vendas WHERE id = ? AND observacao LIKE ?', [
              created.vendaCanceladaId,
              `%${marker}%`
            ])
          ).count
        : 0,
...
async function main() {
  ...
  try {
    ...
    const venda = await request(baseUrl, '/pdv/vendas', {
      method: 'POST',
      token,
      body: JSON.stringify({
        ...
        desconto: 0,
        observacao: marker,
        itens: [{ produto_id: created.produtoId, quantidade: 1 }]
      })
    });
    created.vendaId = venda.data.data.id;
    assert.equal(venda.data.data.status, 'concluida', 'venda PDV deve ser concluida');

    const historicoPdv = await request(
      baseUrl,
      `/pdv/vendas?status=concluida&cliente_id=${created.clienteId}&termo=${encodeURIComponent('Produto Smoke')}&page=1&limit=5`,
      { token }
    );
    assert.ok(historicoPdv.data.data.items.some((item) => item.id === created.vendaId), 'historico PDV deve listar venda concluida');
    assert.equal(historicoPdv.data.data.pagination.page, 1, 'historico PDV deve retornar pagina atual');
    assert.equal(historicoPdv.data.data.pagination.limit, 5, 'historico PDV deve retornar limite');

    const vendaDetalhe = await request(baseUrl, `/pdv/vendas/${created.vendaId}`, { token });
    assert.equal(vendaDetalhe.data.data.id, created.vendaId, 'detalhe PDV deve retornar venda');
    assert.ok(vendaDetalhe.data.data.itens.length >= 1, 'detalhe PDV deve retornar itens');

    await expectPdf(baseUrl, `/pdv/vendas/${created.vendaId}/recibo`, token, 'PDF de recibo PDV');

    const vendaCancelada = await request(baseUrl, '/pdv/vendas', {
      method: 'POST',
      token,
      body: JSON.stringify({
        cliente_id: created.clienteId,
    ...
    created.vendaCanceladaId = vendaCancelada.data.data.id;

    const produtoAntesCancelamento = await request(baseUrl, `/produtos/${created.produtoId}`, { token });
    await request(baseUrl, `/pdv/vendas/${created.vendaCanceladaId}/cancelar`, {
      method: 'POST',
      token,
      body: JSON.stringify({ motivo: marker })
    });
    const produtoDepoisCancelamento = await request(baseUrl, `/produtos/${created.produtoId}`, { token });
    assert.equal(
      Number(produtoDepoisCancelamento.data.data.quantidade),
      Number(produtoAntesCancelamento.data.data.quantidade) + 1,
      'cancelamento PDV deve devolver estoque'
    );

    const historicoCanceladas = await request(baseUrl, `/pdv/vendas?status=cancelada&cliente_id=${created.clienteId}&page=1&limit=5`, { token });
    assert.ok(historicoCanceladas.data.data.items.some((item) => item.id === created.vendaCanceladaId), 'historico PDV deve listar venda cancelada');
      ...
      JSON.stringify(
        {
          ...
          checked: [
            ...
            'estoque manual',
            'PDV venda, historico, busca e recibo',
            'PDV cancelamento com estorno',
